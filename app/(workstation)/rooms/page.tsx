...
export default function RoomsPage() {
  ...
  // Initial fetch + 10s poll
  useEffect(() => {
    const timeoutId = setTimeout(fetchRooms, 0);
...
function RoomCard({
  ...
  // Tick every minute to keep elapsed time fresh without a page-level interval
  useEffect(() => {
    const id = setInterval(() => setNow(new Date()), 60000);
    return () => clearInterval(id);
  }, []);

  const inRoomMins = room.activeVisit
    ? Math.max(0, Math.floor((now.getTime() - new Date(room.activeVisit.calledTime).getTime()) / 60000))
    : 0;
  ...
  return (
    <article
      className={`rounded-lg border p-4 flex flex-col gap-3 ${
        isFree
          ? "bg-white border-neutral-slate-200"
          : "bg-[#F8FAFF] border-[#BFDBFE]"
      }`}
        ...
        {!isFree && room.activeVisit ? (
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-1 text-[12px]">
              <Clock className="w-3.5 h-3.5 text-neutral-slate-400 shrink-0" strokeWidth={1.75} />
  ...
  return (
    ...
    >
      {Array.from({ length: 4 }).map((_, i) => (
        <div
          key={i}
          className="rounded-lg border border-neutral-slate-200 bg-white p-4 space-y-3 animate-pulse"
        >
          <div className="h-4 w-2/3 bg-neutral-slate-200 rounded" />
          <div className="h-3 w-1/2 bg-neutral-slate-100 rounded" />
          <div className="h-10 w-full bg-neutral-slate-100 rounded" />
          <div className="h-8 w-full bg-neutral-slate-200 rounded" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center space-y-3">
      ...
      <p className="text-xs text-neutral-slate-500 max-w-xs">
        No consultation rooms have been set up for this zone yet. Contact an administrator to add rooms.
      </p>
    </div>
  );
}
